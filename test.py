...
def train(args, bit):
    ...
    for epoch in range(args.epoch):
        ...
        for image, tag, tlabel, label, ind in train_loader:
            ...
            if clean_samples.sum() > 0:
                ...
                aff_label = torch.Tensor(aff_label).to(device) 
                ...
                #loss = similarity_loss * args.param_sim + recon_loss +  constr_loss * args.param_sup + losssign * args.param_sign
                loss = similarity_loss * args.param_sim + recon_loss + code_cen_loss * args.param_it + constr_loss * args.param_sup 
    ...
    log.info("Training time: %.2f seconds" % training_time)  # Log training time
...
def eval(moodel, args):
    ...
    MAP50 = calculate_top_map(qu_B=qu_B.numpy(), re_B=re_B.numpy(), qu_L=qu_L.numpy(), re_L=re_L.numpy(), topk=50)
    print('MAP50:', MAP50)
    end_time = time.time()  # End timing evaluation
    evaluation_time = end_time - start_time
    log.info("Evaluation time: %.2f seconds" % evaluation_time)  # Log evaluation time
